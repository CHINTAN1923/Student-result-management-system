...
if(!empty($_POST["classid"])) 
{
 ...
 $stmt = $dbh->prepare("SELECT StudentName,StudentId FROM tblstudents WHERE DeptId= :id order by StudentName");
...
if(!empty($_POST["classid1"])) 
{
 ...
 $status=0;	
 $stmt = $dbh->prepare("SELECT tblsubjects.SubjectName,tblsubjects.id FROM tbldeptsubjectcomb join  tblsubjects on  tblsubjects.id=tbldeptsubjectcomb.SubjectId WHERE tbldeptsubjectcomb.DeptId=:cid and tbldeptsubjectcomb.status!=:stts order by tblsubjects.SubjectName");
...
$id=$dta[0];
$id1=$dta[1];
 $query = $dbh->prepare("SELECT StudentId,DeptId FROM tblresult WHERE StudentId=:id1 and DeptId=:id ");
